...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  //Notes: NUM_VARS size is only 4 and hence we had to make use of collpase clause to increase parallelism
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  //Notes: Only outer two loops are made parallel as the inner loop has dependencies. Also added private clause as by default the array is copied to global memory which creates data race
  #pragma acc parallel loop collapse(2) private(stencil,d3_vals,vals)
  for (k=0; k<nz; k++) {
    for (i=0; i<nx+1; i++) {
