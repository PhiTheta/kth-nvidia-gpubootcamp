...
contains
  ...
  subroutine compute_tendencies_x(state,flux,tend)
    ...
    !Notes: Collapsed only outer 2 loops as inner 2 loops are dependent on each
    !othe. Also explicitly made arrays as private as they are not sahred and
    !need to be provate per thread
!$acc parallel loop gang vector collapse(2) private(stencil,d3_vals,vals)
    do k = 1 , nz
      do i = 1 , nx+1
    ...
    !! TODO: THREAD ME
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !Use the fluxes to compute tendencies for each cella
    !$acc parallel loop 
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine compute_tendencies_z(state,flux,tend)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !Compute fluxes in the x-direction for each cell
    !$acc parallel loop collapse(2) gang vector private(stencil,d3_vals,vals)
    do k = 1 , nz+1
      do i = 1 , nx
        !Use fourth-order interpolation from four cell averages to compute the value at the interface in question
        do ll = 1 , NUM_VARS
...
!$acc parallel loop collapse(2) 
  ...
  subroutine output(state,etime)
    ...
    if (etime == 0) then
      !Create the file
      call ncwrap( nf90mpi_create( MPI_COMM_WORLD , 'new.nc' , nf90_clobber , MPI_INFO_NULL , ncid ) , __LINE__ )
      ...
      !Open the file
      call ncwrap( nfmpi_open( MPI_COMM_WORLD , 'new.nc' , nf90_write , MPI_INFO_NULL , ncid ) , __LINE__ )
